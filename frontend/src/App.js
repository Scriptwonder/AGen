...
function App() {
    ...
    const [analogies, setAnalogies] = useState([]); // State for analogies
    const [selectedAnalogy, setSelectedAnalogy] = useState(null); // State for selected analogy
    ...
    const handleGenerate = async () => {
        ...
        setError(null);
        try {
            const response = await axios.post('/generate-concept', { concept });
            setResult(response.data);
            setAnalogies([]); // Reset analogies when generating a new concept
        } catch (err) {
    ...
    const handleGenerateAnalogies = async () => {
        //console.log(concept, audience);
        if (!concept) {
          setError('Please enter a concept before generating analogies.');
          return;
        }
        ...
        setError(null);
        try {
            const response = await axios.post('http://127.0.0.1:5000/generate-analogy', { concept, audience });
            setAnalogies(response.data.analogies);
            setSelectedAnalogy(null); // Reset selected analogy
        } catch (error) {
            console.error('Error generating analogies:', error);
            setError('Failed to generate analogies. Please try again.');
        } finally {
            setLoading(false);
        }
    };

    const handleSelectAnalogy = (index) => {
        setSelectedAnalogy(analogies[index]);
    };
...
//         <p>{graphData.concept}</p>
//         <p>{graphData.analogy}</p>
//         <GraphVisualizer
